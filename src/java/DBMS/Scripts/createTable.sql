...
CREATE TABLE incluye (
    CODIGO_DECANATO VARCHAR(10) REFERENCES DECANATO (CODIGO) ON DELETE CASCADE,
    CODIGO_CARRERA  VARCHAR(10) REFERENCES CARRERA (CODIGO) ON DELETE CASCADE,

    CONSTRAINT PK_incluye PRIMARY KEY (CODIGO_DECANATO,CODIGO_CARRERA)
);

CREATE TABLE dicta (
    USBID_PROFESOR VARCHAR(15) REFERENCES PROFESOR (USBID) ON DELETE CASCADE,
    CODIGO_MATERIA VARCHAR(10) REFERENCES MATERIA (CODIGO) ON DELETE CASCADE,

    CONSTRAINT PK_dicta PRIMARY KEY (USBID_PROFESOR,CODIGO_MATERIA)
);

CREATE TABLE se_adscribe (
    CODIGO_COORDINACION VARCHAR(10) REFERENCES COORDINACION (CODIGO) ON DELETE CASCADE,
    CODIGO_DEPARTAMENTO VARCHAR(10) REFERENCES DEPARTAMENTO (CODIGO) ON DELETE CASCADE,
...
CREATE TABLE oferta (
    CODIGO_MATERIA VARCHAR(10) REFERENCES MATERIA (CODIGO) ON DELETE CASCADE,
    CODIGO_DEPARTAMENTO VARCHAR(10) NOT NULL REFERENCES DEPARTAMENTO (CODIGO) ON DELETE CASCADE,

    CONSTRAINT PK_oferta PRIMARY KEY (CODIGO_MATERIA,CODIGO_DEPARTAMENTO)
);

CREATE TABLE pertenece ( 
    USBID_PROFESOR VARCHAR(10) REFERENCES PROFESOR (USBID) ON DELETE CASCADE,
    CODIGO_DEPARTAMENTO VARCHAR (10) REFERENCES DEPARTAMENTO (CODIGO) ON DELETE CASCADE,
...
CREATE TABLE suscribe (
    CODIGO_DEPARTAMENTO VARCHAR(10) REFERENCES DEPARTAMENTO (CODIGO) ON DELETE CASCADE,
    CODIGO_DECANATO VARCHAR(10) NOT NULL REFERENCES DECANATO (CODIGO) ON DELETE CASCADE,
...
ALTER TABLE incluye ADD CONSTRAINT Fk_incluye_decanato
    FOREIGN KEY (CODIGO_DECANATO) REFERENCES DECANATO (CODIGO);

-- ALTER TABLE incluye ADD CONSTRAINT Fk_incluye_carrera
